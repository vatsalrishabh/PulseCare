...
const ManageMulAppointment = ({ selectedDisease, selectedDoctor }) => {
  ...
  const [dates, setDates] = useState([]); // the 1 point where json from get request after filtering today's slot comes
  const [selectedSlot, setSelectedSlot] = useState(null); // 3 checkbox selected dates are storeed here once cliked on check box
  const [displayedDates, setDisplayedDates] = useState([]); // is just make sures how many dates are being dispalyed not that usefull
  const [minnesotaDates, setMinnesotaDates] = useState([]); //the 2 point minnesota dates and time is added to json
  const [checkBoxSlots, setCheckBoxSlots] = useState([]);
  ...
  function addMinnesotaDateTime(bookings) {
    bookings.forEach((booking) => {
      booking.slots.forEach((slot) => {
        ...
        const minnesotaDateTime = new Date(
          originalDateTime.getTime() - (10 * 60 * 60 * 1000 + 30 * 60 * 1000)
        );

        // Set new fields
        slot.minnesotaTime = minnesotaDateTime.toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        });
  ...
  const fetchBookings = async () => {
    try {
      ...
      const updatedDates = addMinnesotaDateTime(fetchedDates); //  2nd step is that minnesota date and time is added in each slots which came  from server
  ...
  const handleSlotSelect = (slot, date, isChecked) => {
    // Ensure selectedSlot is not null or undefined before accessing bookingId
    if (selectedSlot && selectedSlot.bookingId) {
      console.log(selectedSlot.bookingId + " selectedSlot");
    } else {
      console.log("No selectedSlot or bookingId available");
    }
  
    setSelectedSlot({ ...slot, date });
  
    setCheckBoxSlots((prevSlots) => {
      if (isChecked) {
        return [
          ...prevSlots,
          {
        ...
        return prevSlots.filter(
          (s) => s.bookingId !== slot.bookingId || s.date !== date
        );
      }
    });
  
    console.log(checkBoxSlots);
  };
  

  const openModalAllSelected = () => {
    setOpenModal(true);
  };
  ...
  const handleBookingConfirm = async () => {
    try {
      checkBoxSlots
      await axios.put(`${BaseUrl}/api/patients/postMultipleBookingsAdmin`, {
        checkBoxSlots,
      });
  ...
  return (
    <>
      ...
      <h1 className="ms-3 text-sm font-medium text-gray-900 dark:text-gray-300 p-4 flex justify-end">
        ...
        <div className="px-1">
          <Button onClick={openModalAllSelected} color="amber">
            {" "}
            Send Locked Slots
          </Button>
        </div>
        <div className="px-1">
          <Button onClick={gotoSelectApp} color="blue">
            Single Select
          </Button>
        </div>
        <div className="px-1">
                ...
                <ul>
                  {slots.map((slot, idx) => (
                    ...
                    >
                      ...
                      <span className={`${!hiddenUSA ? "hidden" : ""}`}>
                        {slot.time}
                      </span>
                      <br />
                      {/* when toggeled for INdian slots display onley these two */}
                      ...
                      <input
                        ...
                        id=""
                        onChange={(e) =>
                          handleSlotSelect(slot, date, e.target.checked)
                        }
                      />
                      <span
      ...
      <Modal open={openModal} onClose={() => setOpenModal(false)}>
        <div className="p-6 bg-white shadow-lg rounded-md w-80 mx-auto mt-20">
          ...
          <div className="mt-4 flex justify-between">
            ...
            <button
              className="bg-custom-maroon0 text-white px-4 py-2 rounded"
